...
def get_weighted_w2v(seq, w2v_model, tfidf_dict):
    vecs = []
    for token in seq:
        if token in w2v_model.wv and token in tfidf_dict:
            vec = w2v_model.wv[token] * tfidf_dict[token]
            vecs.append(vec)
    if len(vecs) == 0:
...
if __name__ == "__main__":
    ...
    (x_train_1, y_train), (x_test_1, y_test), data_df = dataloader.load_HDFS(struct_log,
                                                                event_traces, 
                                                                label_file=label_file,
                                                                window='session', 
                                                                train_ratio=0.9,
    ...
    result_1 = np.concatenate((x_train_1, x_test_1), axis=0)
    
    sequences = [list(map(str, row)) for row in result.tolist()]

    sequences_1 = [list(map(str, row)) for row in result_1.tolist()]
    ...
    x_train_1 = feature_extractor.fit_transform(x_train_1, term_weighting='tf-idf', 
    ...
    # Tag sequences for training (required by Doc2Vec)
    ...
    X_train = np.array([d2v_model.dv[str(i)] for i in range(len(x_train))])

    X_test = np.array([d2v_model.dv[str(i)] for i in range(len(x_test))])
    '''
