...
if __name__ == "__main__":
    ...
    model = BiLSTMAutoencoder(input_size=48, hidden_size=128, latent_size=64).to(device)
    model = model.double()
    ...
    for epoch in range(10):
        ...
        model.eval()
        with torch.no_grad():
            inputs = torch.tensor(x_test, dtype=torch.float64).to(device)
            outputs = model(inputs)
        ...
        #thresholds = np.linspace(t0 - 0.1, t0 + 0.1, 1000)
